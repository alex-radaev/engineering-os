...
Until there is a dedicated CLI writer, persist the design as a run brief so it shows up in future wake-up context:

- `node "${CLAUDE_PLUGIN_ROOT}/scripts/crew.mjs" write-run-brief --repo "$PWD" --title "design: <short title>" --goal "<summary>" --mode "assisted single-session" --pace "<pace>"`

Then save the full design body under `.claude/artifacts/crew/designs/<slug>.md` so the builder and reviewer can read it in the next session.
...
- the handoff should include the design doc path in the `evidence or changed files` field
