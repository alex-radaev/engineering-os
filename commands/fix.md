...
25a. Before invoking `write-final-synthesis` with `--mission-terminal-status` in {`done`, `partial`}, ensure a reviewer subagent has run and step 22's `gate phase:review` event was emitted (and is newer than the most recent `phase=implementation` event). The CLI enforces this server-side: it exits 2 with `crew: rule #9 violated …` otherwise. Bypass terminal statuses are `abandoned`, `needs_user`, `blocked` — they signal incomplete work, not a skipped review.
