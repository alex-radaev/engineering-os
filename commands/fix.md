---
description: Investigate and fix broken behavior (alias for investigate-bug).
---
...
1a. If the envelope is present, capture `mission_id`, `reporting.status_file`, `reporting.event_log`, and `reporting.handoff_file` from the parsed envelope. Pass them explicitly on every mission writer call below — there is no pointer-file fallback (see `workflow.md § Mission Reporting`). Then fire the start event:
    - `node "${CLAUDE_PLUGIN_ROOT}/scripts/crew.mjs" append-mission-event --repo "$PWD" --mission-id <envelope.mission_id> --event-log <envelope.reporting.event_log> --event started --phase research --summary "<goal>"`.
...
21. When a helper or teammate returns meaningful evidence or ownership changes, write a handoff artifact if the run is substantial:
   ...
   - If an envelope is active, fire `append-mission-event --mission-id <envelope.mission_id> --event-log <envelope.reporting.event_log> --event progress --phase <research|implementation> --summary "<what changed>"` before/after the transition.
...
25a. Before invoking `write-final-synthesis` with `--mission-terminal-status` in {`done`, `partial`}, ensure a reviewer subagent has run and step 22's `gate phase:review` event was emitted (and is newer than the most recent `phase=implementation` event). The CLI enforces this server-side: it exits 2 with `crew: rule #9 violated …` otherwise. Bypass terminal statuses are `abandoned`, `needs_user`, `blocked` — they signal incomplete work, not a skipped review. Research-only escape hatch: if the investigation closed without any code changes (a handoff with findings and a recommendation, no diff), pass `--no-code-changes` to skip the review-gate check. The CLI rejects this flag if the event log contains any `phase=implementation` event — once you've coded, you can't opt out.
