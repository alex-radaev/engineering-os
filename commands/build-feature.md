...
17. If this run was driven by a design doc (from `/crew:design` or user-referenced), pass the design doc path to both the builder and the reviewer in their handoffs so they work from the same spec. If there is no design doc, say "no design doc" explicitly — stale docs under `designs/` would mislead the specialist if it went looking.
18. If using multiple builders, ensure write scopes are disjoint before running them in parallel.
19. If using a `team run`, claim files only when parallel work might collide, and open approvals only when scope or ownership boundaries must be crossed.
20. If using a `team run`, assign bounded work to:
...
21. Keep ownership explicit and avoid same-file parallel editing.
22. Completed implementation tasks should go through reviewer review before they are treated as done. Reviewer should treat missing or weak automated coverage as a default rejection unless a concrete low-risk deferral reason is documented. If review is skipped, say so explicitly and justify it before the final synthesis.
23. If the result has user-visible, system-visible, or externally observable behavior that can be exercised meaningfully, validator validation is the default at meaningful milestones or after integration.
24. If the work is already moving through a deployment boundary, recommend or enter `/crew:ship` instead of treating local implementation as the whole workflow.
25. When a helper or teammate hands work back, write a handoff artifact if the run is substantial:
   - `node "${CLAUDE_PLUGIN_ROOT}/scripts/crew.mjs" write-handoff --repo "$PWD" --title "<short title>" ...`
26. When a reviewer materially validates the work, write a review artifact:
   - `node "${CLAUDE_PLUGIN_ROOT}/scripts/crew.mjs" write-review-result --repo "$PWD" --title "<short title>" ...`
27. When a validator materially validates the behavior, write a validation artifact:
   - `node "${CLAUDE_PLUGIN_ROOT}/scripts/crew.mjs" write-validation-result --repo "$PWD" --title "<short title>" ...`
28. End with a clear synthesis for the user:
   - what changed
   - what was reviewed
   - what was validated
...
29. For substantial work, write a final synthesis artifact:
